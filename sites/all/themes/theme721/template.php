...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for theme721
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('theme721_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function theme721_html_head_alter(&$head_elements) {
 ...
 */
function theme721_preprocess_search_block_form(&$vars) {
 ...
 */
function theme721_preprocess(&$vars, $hook) {
  $vars['theme721_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function theme721_preprocess_html(&$vars) {
  
  $vars['doctype'] = _theme721_doctype();
  $vars['rdf'] = _theme721_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = theme721_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = theme721_id_safe('section-' . $section);
 ...
 */
function theme721_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'theme721') . '/css/maintenance-page.css');
}
 ...
 */
function theme721_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function theme721_preprocess_block(&$vars, $hook) {
 ...
 */
function theme721_menu_tree($vars) {
 ...
 */
function theme721_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function theme721_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('theme721_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('theme721_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('theme721_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('theme721_breadcrumb_separator');
 ...
 * @return bool
 */
function theme721_tabs_float() {
  $float = (bool) theme_get_setting('theme721_tabs_float');
  $float_node = (bool) theme_get_setting('theme721_tabs_node');
...
function theme721_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function theme721_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = theme721_id_safe($element['#title']);
 ...
 */
function theme721_preprocess_menu_local_task(&$vars) {
 ...
 */
function theme721_menu_local_tasks(&$vars) {
  $output = '';
