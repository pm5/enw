...
/**
 * @file
 * Contains theme override functions and process & preprocess functions for enwtheme
 */

// Auto-rebuild the theme registry during theme development.
if (theme_get_setting('enwtheme_clear_registry')) {
  // Rebuild .info data.
 ...
 */
function enwtheme_html_head_alter(&$head_elements) {
 ...
 */
function enwtheme_preprocess_search_block_form(&$vars) {
 ...
 */
function enwtheme_preprocess(&$vars, $hook) {
  $vars['enwtheme_path'] = base_path() . path_to_theme();
}

/**
 * Implements template_preprocess_html().
 */
function enwtheme_preprocess_html(&$vars) {
  
  $vars['doctype'] = _enwtheme_doctype();
  $vars['rdf'] = _enwtheme_rdf($vars);
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = enwtheme_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = enwtheme_id_safe('section-' . $section);
 ...
 */
function enwtheme_preprocess_page(&$vars) {
 ...
 */
function enwtheme_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'enwtheme') . '/css/maintenance-page.css');
}
 ...
 */
function enwtheme_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function enwtheme_preprocess_block(&$vars, $hook) {
 ...
 */
function enwtheme_menu_tree($vars) {
 ...
 */
function enwtheme_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function enwtheme_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('enwtheme_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('enwtheme_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('enwtheme_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('enwtheme_breadcrumb_separator');
 ...
 * @return bool
 */
function enwtheme_tabs_float() {
  $float = (bool) theme_get_setting('enwtheme_tabs_float');
  $float_node = (bool) theme_get_setting('enwtheme_tabs_node');
...
function enwtheme_id_safe($string) {
  // Strip accents
 ...
 * @ingroup themeable
 */
function enwtheme_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = enwtheme_id_safe($element['#title']);
 ...
 */
function enwtheme_preprocess_menu_local_task(&$vars) {
 ...
 */
function enwtheme_menu_local_tasks(&$vars) {
  $output = '';
